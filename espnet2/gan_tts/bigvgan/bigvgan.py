...
class BigVGANGenerator(torch.nn.Module):
    def __init__(self, initial_channel, out_channels, resblock_kernel_sizes, resblock_dilation_sizes, upsample_rates, upsample_initial_channel, upsample_kernel_sizes, gin_channels=0, rank=0):
        super(BigVGANGenerator, self).__init__()
        self.num_kernels = len(resblock_kernel_sizes)
        self.num_upsamples = len(upsample_rates)
        self.conv_pre = Conv1d(initial_channel, upsample_initial_channel, 7, 1, padding=3)
        resblock = BigVGANAMPBlock
...
class BigVGANDiscriminatorP(torch.nn.Module):
    def __init__(self, period, kernel_size=5, stride=3, use_spectral_norm=False):
        super(BigVGANDiscriminatorP, self).__init__()
        self.period = period
...
class BigVGANDiscriminatorR(torch.nn.Module):
    def __init__(self, resolution, use_spectral_norm=False):
        super(BigVGANDiscriminatorR, self).__init__()
        norm_f = weight_norm if use_spectral_norm == False else spectral_norm
...
class BigVGANMultiPeriodDiscriminator(torch.nn.Module):
    def __init__(self, use_spectral_norm=False):
        super(BigVGANMultiPeriodDiscriminator, self).__init__()
        periods = [2,3,5,7,11]
        resolutions = [[1024, 120, 600], [2048, 240, 1200], [512, 50, 240]]

        discs = [BigVGANDiscriminatorR(resolutions[i], use_spectral_norm=use_spectral_norm) for i in range(len(resolutions))]
        discs = discs + [BigVGANDiscriminatorP(i, use_spectral_norm=use_spectral_norm) for i in periods]
