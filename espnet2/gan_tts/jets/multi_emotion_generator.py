...
from espnet2.gan_tts.other_vocoder import  Multiband_iSTFT_Generator, Multistream_iSTFT_Generator
...
class JETSGenerator(torch.nn.Module):
    ...
    def __init__(
        ...
        self.upsample_factor = int(np.prod(generator_upsample_scales) * generator_out_channels)
        self.idim = idim
        self.odim = odim
        ...
        if generator_type == "hifigan":
        ...
        elif generator_type == "multiband_istft_hifigan":
            self.generator = Multiband_iSTFT_Generator(
                initial_channel=adim,
                resblock='1',
                resblock_kernel_sizes=generator_resblock_kernel_sizes,
                resblock_dilation_sizes=generator_resblock_dilations,
                upsample_rates=generator_upsample_scales,
                upsample_initial_channel=generator_channels,
                upsample_kernel_sizes=generator_upsample_kernel_sizes,
                gen_istft_n_fft=16,
                gen_istft_hop_size=4,
                subbands=4,
                gin_channels=generator_global_channels,
            )
            self.upsample_factor *= 4
        elif generator_type == "multistream_istft_hifigan":
            self.generator = Multistream_iSTFT_Generator(
                initial_channel=adim,
                resblock='1',
                resblock_kernel_sizes=generator_resblock_kernel_sizes,
                resblock_dilation_sizes=generator_resblock_dilations,
            ...
            raise ValueError(f"{generator_type} type not supported in jets currently.")
