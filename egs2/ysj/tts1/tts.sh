...
dataset="" #dataset to be passed to local/data.sh.
...
Options:
    ...
    # Data prep related
    --dataset # dataset to be passed to local/data.sh (default="").
...
if [ -z "${tts_stats_dir}" ]; then
    ...
    #     tts_stats_dir+="_${g2p}"
    # fi
    tts_stats_dir="${expdir}/tts_stats_${tag}"
fi
...
if ! "${skip_data_prep}"; then
    ...
    if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
        ...
        for dset in "${train_set}" "${valid_set}" ${test_sets}; do
            ...
            utils/copy_data_dir.sh data/$dataset/"${dset}" "${data_feats}${_suf}/${dset}"
            rm -f ${data_feats}${_suf}/${dset}/{segments,wav.scp,reco2file_and_channel}
            _opts=
            if [ -e data/$dataset/"${dset}"/segments ]; then
                _opts+="--segments data/$dataset/${dset}/segments "
            fi
            # shellcheck disable=SC2086
            scripts/audio/format_wav_scp.sh --nj "${nj}" --cmd "${train_cmd}" \
                --audio-format "${audio_format}" --fs "${fs}" ${_opts} \
                "data/$dataset/${dset}/wav.scp" "${data_feats}${_suf}/${dset}"
            echo "${feats_type}" > "${data_feats}${_suf}/${dset}/feats_type"
        done

        # Extract X-vector
        if "${use_xvector}"; then
            log "Stage 2+: Extract X-vector: data/$dataset/ -> ${dumpdir}/xvector (Require Kaldi)"
            ...
            for dset in "${train_set}" "${valid_set}" ${test_sets}; do
                ...
                utils/copy_data_dir.sh "data/$dataset/${dset}" "${dumpdir}/mfcc/${dset}"
        ...
        if "${use_sid}"; then
            log "Stage 2+: Prepare speaker id: data/$dataset/ -> ${data_feats}/"
            for dset in "${train_set}" "${valid_set}" ${test_sets}; do
                if [ "${dset}" = "${train_set}" ] || [ "${dset}" = "${valid_set}" ]; then
                    _suf="/org"
                else
        ...
        if "${use_lid}"; then
            log "Stage 2+: Prepare lang id: data/$dataset/ -> ${data_feats}/"
            for dset in "${train_set}" "${valid_set}" ${test_sets}; do
                if [ "${dset}" = "${train_set}" ] || [ "${dset}" = "${valid_set}" ]; then
                    _suf="/org"
                else
...
if ! "${skip_train}"; then
    if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
        ...
        if [ -n "${teacher_dumpdir}" ]; then
            python pyscripts/utils/fix_durations_mismatch.py --dataset $dataset --tts_stat_dir "${tts_stats_dir}" --train_set "${train_set}" --valid_set "${valid_set}"
        fi
    fi
